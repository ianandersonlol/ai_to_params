#!/usr/bin/env python
"""
Core utilities for Rosetta parameterization from molfile_to_params.py
Extracted for reuse in ai_to_params.py
...
class r3:
    ...
    def distance(a, b):
        if hasattr(a, 'x') and hasattr(b, 'x'):
            ...
            raise ValueError("Objects must have x,y,z coordinates")

    @staticmethod
    def from_to(a, b):
        return r3.Triple(b.x - a.x, b.y - a.y, b.z - a.z)

    @staticmethod
    def add(a, b):
        return r3.Triple(a.x + b.x, a.y + b.y, a.z + b.z)
    ...
    def mult(a, scalar):
        ...
        return r3.Triple(a.x * scalar, a.y * scalar, a.z * scalar)

    @staticmethod
    def centroid(points):
        """Calculate centroid of a list of points"""
        if not points:
            return r3.Triple(0, 0, 0)
        ...
        return r3.Triple(sum_x/n, sum_y/n, sum_z/n)
    ...
    def dihedral(a, b, c, d):
        ...
        def normalize(v):
            ...
            if mag < 1e-10:
                return r3.Triple(0, 0, 0)
            return r3.Triple(v.x/mag, v.y/mag, v.z/mag)
...
class MolfileAtom:
    def __init__(self):
        ...
        self.is_ring = False
        self.ring_size = 0
        # Additional fields will be added by add_fields_to_atoms()

class MolfileBond:
    def __init__(self):
        self.a1 = None  # First atom
        self.a2 = None  # Second atom
        self.order = Bond.SINGLE
        self.mirror = None  # Bidirectional bond reference
        self.is_ring = False
        # Additional fields will be added by add_fields_to_bonds()
...
def setup_amino_acid(atoms):
    ...
    if not all( name in atom_names for name in atom_prio.keys() ):
        raise ValueError("Input is missing essential atom(s) for amino acids: %s" % ", ".join(atom_prio.keys()))
...
def build_fragment_trees(molfile):
    ...
    for frag_id in set([a.fragment_id for a in molfile.atoms]):
        ...
        #    parent.children.sort(key=lambda a: a.is_H)
...
def dijkstra(start, nodes, nbr, dist):
    ...
    while len(queue) > 0:
        curr_idx = argmin([q[DIST] for q in queue]) # on first pass this is start
...
def index_atoms(atoms):
    '''Helper function to create mapping from atom to its 1-based index'''
    return dict((atom, i+1) for i, atom in enumerate(atoms))